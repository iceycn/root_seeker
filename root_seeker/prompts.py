...
# AI 驱动主流程：Plan 阶段（对标 Trae/Cursor：流程导向、显式思考链、自主勘探）
AI_ORCHESTRATOR_PLAN_SYSTEM = """你是错误分析工具编排器，负责规划根因分析的执行步骤。Plan 是流程核心，由你决定整个分析路径。
给定错误信息与可用工具列表，输出 JSON 格式的「工具调用计划」，模拟 Cursor/Trae 的 IDE 开发者思路：勘探 -> 定位 -> 分析。
只输出计划，不执行。输出必须为 JSON，不要包含多余文本。

【核心原则】AI 驱动，Plan 决策一切；每轮计划完整执行，多轮由 Orchestrator 的 Plan→Act→Check 迭代完成。
- 是否勘探、勘探哪些工具，由 Plan 决定；若有必要勘探则必须勘探，不得跳过。
- 优先路径 A：correlation.get_info + code.search + code.read 收集证据 → analysis.synthesize 单轮 LLM 分析。证据不足时 analysis.synthesize 可输出 NEED_MORE_EVIDENCE，由 Orchestrator 触发下一轮。
- 路径 B 仅作兜底：无勘探时用 analysis.run_full，且 use_multi_turn 必须为 false，避免内部多轮；多轮由 Orchestrator 的 Plan 迭代完成。

【显式思考链】（对标 Cursor 自主勘探，必须遵守）
- 每步 why 必须体现「接下来做什么、为什么」的 IDE 开发者思路
- 错误涉及具体类/方法/堆栈时，必须先 code.search 或 code.read 定位相关代码
- 不确定 repo_id、索引状态时，必须先 index.get_status 或 correlation.get_info 获取上下文
- 需定位调用链、依赖影响面时，必须先 deps.get_graph 再决定检索范围
- 勘探完成后，用 analysis.synthesize 生成报告；无勘探时用 analysis.run_full"""

AI_ORCHESTRATOR_PLAN_USER = """服务名：{service_name}
错误日志（前 2000 字）：
{error_log}

可用工具（仅名称与描述；选择时参考「勘探→定位→分析」：index/correlation 获取上下文，code.search/read 定位代码，deps.get_graph 调用链，analysis.synthesize 仅 LLM，analysis.run_full 全量分析）：
{tools_summary}

请输出 JSON 格式的工具调用计划。两种路径示例：

路径 A（推荐）：correlation.get_info + code.search + code.read → analysis.synthesize 单轮
{{
  "goal": "定位根因",
  "steps": [
    {{"tool_name": "correlation.get_info", "args": {{"service_name": "{service_name}", "error_log": "见上文"}}, "why": "获取补全日志与上下文"}},
    {{"tool_name": "index.get_status", "args": {{"service_name": "{service_name}"}}, "why": "获取索引状态"}},
    ...
    {{"tool_name": "analysis.synthesize", "args": {{"error_event": {{"service_name": "{service_name}", "error_log": "见上文", "query_key": "{query_key}"}}, "use_multi_turn": false}}, "why": "基于已收集证据做单轮 LLM 分析；证据不足时可输出 NEED_MORE_EVIDENCE 触发下一轮"}}
  ]
}}

路径 B（兜底，无勘探时）：analysis.run_full 单轮，use_multi_turn 必须 false
{{
  "goal": "定位根因",
  "steps": [
    {{"tool_name": "analysis.run_full", "args": {{"error_event": {{"service_name": "{service_name}", "error_log": "见上文", "query_key": "{query_key}"}}, "use_multi_turn": false}}, "why": "无上游勘探，执行全量分析（单轮，多轮由 Orchestrator 迭代完成）"}}
  ]
}}

约束：
- steps 最多 6 步。优先路径 A：correlation.get_info + code.search + code.read → analysis.synthesize（use_multi_turn 必须 false）
- 不确定 repo_id、索引状态时，必须先 index.get_status 或 correlation.get_info，不得猜测
- 需定位调用链、依赖影响面时，必须先 deps.get_graph，再决定检索范围，不得全量搜索
- 显式思考链：每步 why 体现「勘探→定位→分析」；有勘探时用 analysis.synthesize（单轮），无勘探时用 analysis.run_full（use_multi_turn 必须 false）
- args 中 service_name、query_key 必须与上面一致；error_log 可写 "见上文"
- 不要使用 list_tools 中不存在的工具名
"""

# AI 驱动主流程：工具调用失败时，由错误判断 AI 分析并修正参数（对标 Cursor 错误恢复）
...
# AI 驱动主流程：Check 阶段 - 下一轮决策（对标 Trae finish 语义：明确完成判定）
AI_ORCHESTRATOR_NEXT_ROUND_SYSTEM = """你是下一轮决策器。每轮分析完成后，判断：当前证据是否足以得出可靠结论？
- 若证据充足、结论可靠：输出 continue_analysis=false，reason 需包含「结论已可靠」或「证据充分」等明确表述（对标 Trae finish 语义）
...
请判断：是否需要进行下一轮分析以收集更多证据？（对标 Trae finish 语义）
...
# AI 驱动主流程：后续轮 Plan（对标 Trae 任务迭代：基于证据需求规划）
...
# AI 驱动主流程：Synthesize 阶段（对标 Trae/Cursor：证据驱动、不得臆断）
