...
位于 `ai-log-helper` 目录下，与 `root_seeker` 平级：`ai-log-helper/ruoyi-rootseeker-admin/`。
...
**方式一（推荐）**：在 ai-log-helper 根目录一键启动全部服务（含 RootSeeker、Admin、Qdrant、Zoekt）：
...
```bash
cd ruoyi-rootseeker-admin   # 在 ai-log-helper 根目录下执行
